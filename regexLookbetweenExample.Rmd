...
naicsCodes
```
We're getting there, but not quite. We don't want that annoying 'Name: ' in front of the company, and it'd be nice to have the NAICS codes as separate entires in a vector. We can achieve that with the following.
```{r}
cleanerCompany = str_extract(company, "(?<=Name: )(.*)")
naicsVector = str_split(naicsCodes, ",", simplify = TRUE)
cleanerCompany
naicsVector
```
